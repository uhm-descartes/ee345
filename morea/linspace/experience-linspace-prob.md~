...
5. Not all questions here are about linear spaces, some are a recap
   from Gaussian elimination. 
   
     * If $$A$$ is an invertible 4x4 matrix, what is the rref of
       $$\begin{bmatrix} A & A \end{bmatrix}$$ and $$\begin{bmatrix}
       A\\ A \end{bmatrix}$$? What about $$\begin{bmatrix} A & AB
       \end{bmatrix}$$? (You do not need to know $$A$$ to write
       the answers out). 
